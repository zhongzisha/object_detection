...
V100 2GPU, bs=8 (focal loss from tensorflow/tpu/retinanet sigmoid_focal_loss3)
...
import fcos_model as model

COMMON_POSTFIX = 'train_FCOS_sigmoid_focal_loss4'
...
def train():
    ...
    if num_gpus > 1:
        ...
        base_inputs_list = [tf.split(value, num_or_size_splits=num_gpus, axis=0)
                            for value in [images, gt_boxes, gt_labels, orig_gt_counts]]

        tower_grads = []
        total_loss_dict = {'cls_loss': tf.constant(0.), 'reg_loss': tf.constant(0.),
        ...
        if step % cfg.TRAIN.SAVE_SUMMARY_STEPS == 0:
            ...
            print(
                ...
                '{:.2f} s/step, {:.2f} samples/s'.format(
                    step, lr_, tl_,
                    cls_loss_, reg_loss_, centerness_loss_, wd_loss_,
    ...
    for idx, roidb in enumerate(roidbs):
        ...
        for c in range(1, 81):
            ...
            if len(inds) > 0:
                ...
                # 类内NMS
                keep = nms(boxes_keep[:, 0:5], thresh=cfg.FRCNN.TEST.NMS_THRESH)
