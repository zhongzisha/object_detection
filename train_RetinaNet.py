...
import retinanet_model as model

COMMON_POSTFIX = 'train_RetinaNet'


def tower_loss_func(inputs, reuse=False):

    with tf.variable_scope('resnet50', reuse=reuse):
        loss_dict = model.model_retinanet(inputs, is_training=False, reuse=reuse,
    ...
    ds = tf.data.Dataset.from_generator(
        lambda: map(lambda x:
                    tuple([x[k] for k in ['images', 'gt_boxes', 'gt_labels', 'orig_gt_counts',
                                          ...
                                          'all_anchors_level7', 'anchor_labels_level7', 'anchor_boxes_level7']]),
                                       data_iter),
                           ...
                           (tf.TensorShape([None, None, None, 3]),
                            ...
                            tf.TensorShape([None, None, None, None]),
                            tf.TensorShape([None, None, None, None]), tf.TensorShape([None, None, None, None, 4])  #lv7
                            ))
    ...
    if num_gpus > 1:
        ...
        fpn_all_anchors_list = \
            [[tf.identity(value) for _ in range(num_gpus)] for value in
             [all_anchors_level3, all_anchors_level4, all_anchors_level5,
              all_anchors_level6, all_anchors_level7]]
        fpn_anchor_gt_labels_list = \
            [tf.split(value, num_or_size_splits=num_gpus, axis=0) for value in
             [anchor_labels_level3, anchor_labels_level4, anchor_labels_level5,
              anchor_labels_level6, anchor_labels_level7]]
        fpn_anchor_gt_boxes_list = \
            [tf.split(value, num_or_size_splits=num_gpus, axis=0) for value in
             [anchor_boxes_level3, anchor_boxes_level4, anchor_boxes_level5,
              anchor_boxes_level6, anchor_boxes_level7]]

        tower_grads = []
        total_loss_dict = {'cls_loss': tf.constant(0.), 'box_loss': tf.constant(0.)}
            ...
            if restore_from_original_checkpoint:
                ...
                for var in tf.global_variables():
                    ...
                    if 'rpn' not in var.name and 'rcnn' not in var.name and 'fpn' not in var.name \
                            and 'global_step' not in var.name and 'retina' not in var.name and \
                            'Momentum' not in var.name and 'ExponentialMovingAverage' not in var.name:
...
def eval_one_dataset(dataset_name, output_filename):
    ...
    with tf.variable_scope('resnet50'):
        final_boxes, final_scores, final_labels, final_inds = \
            model.model_retinanet(images, is_training=False, data_format='channels_last', mode='test')
