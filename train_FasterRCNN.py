...
COMMON_POSTFIX = 'train_FasterRCNN'


def tower_loss_func(inputs, reuse=False):
    with tf.variable_scope('resnet50', reuse=reuse):
        loss_dict = model.model(inputs, is_training=False, reuse=reuse,
...
def train():
    ...
    ds = tf.data.Dataset.from_generator(lambda: map(lambda x: tuple([x[k] for k in
                                                        ['images', 'gt_boxes', 'gt_labels', 'orig_gt_counts',
                                                         'anchor_labels', 'anchor_boxes', 'all_anchors']]),
                                       data_iter),
                           (tf.float32, tf.float32, tf.int64, tf.int32, tf.int32, tf.float32, tf.float32),
    ...
    images, gt_boxes, gt_labels, orig_gt_counts, anchor_labels, anchor_boxes, all_anchors = ds.get_next()
    net_inputs = [images, gt_boxes, gt_labels, orig_gt_counts, anchor_labels, anchor_boxes]
    ...
    if num_gpus > 1:
        ...
        net_inputs_list = [tf.split(value, num_or_size_splits=num_gpus, axis=0) for value in net_inputs]
        ...
        for i, gpu_id in enumerate(gpus):
            with tf.device('/gpu:%d' % gpu_id):
                with tf.name_scope('model_%d' % gpu_id) as scope:
                    ...
                    tower_loss_dict = tower_loss_func([input[i] for input in net_inputs_list] + [all_anchors], reuse=(gpu_id > 0))
        ...
        tower_loss_dict = tower_loss_func(net_inputs + [all_anchors])
...
def eval_one_dataset(dataset_name, output_filename):
    ...
    with tf.variable_scope('resnet50'):
        final_boxes, final_scores, final_labels, final_inds = \
            model.model(images, is_training=False, data_format='channels_last', mode='test')
